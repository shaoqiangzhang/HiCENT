...
class one_conv(nn.Module):
    def __init__(self, inchanels, growth_rate, kernel_size=3, relu=True):
        ...
        if relu:
            ...
            #self.relu = nn.LeakyReLU(negative_slope=0.01)  # ReLU更换为LeakyReLU
...
class BasicConv(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=1, dilation=1, groups=1, relu=True,
        ...
        #self.relu = nn.LeakyReLU(0.01, inplace=True) if relu else None  # ReLU更换为LeakyReLU
...
class CALayer1(nn.Module):
    def __init__(self, channels):
        ...
        self.conv = nn.Conv2d(channels, channels, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0))
...
class Cascading_Block(nn.Module):
    def __init__(self, channels):
        ...
        self.ca1 = CALayer1(channels)
        self.ca2 = CALayer1(channels)
        self.ca3 = CALayer1(channels)
...
class Updownblock(nn.Module):
    def __init__(self, n_feats):
        ...
        self.encoder1 =Cascading_Block(n_feats)
    ...
    def forward(self, x):
        ...
        x2 = self.encoder2(x1)
        x3 = self.down(x2)
        high = x1 - F.interpolate(x3, size=x.size()[-2:], mode='bilinear', align_corners=True)
        # print("ARFB input shape:{}".format(x2.shape))
        for i in range(5):
            x3 = self.decoder_low(x3)
        x4 = x3
        # print("ARFB output shape:{}".format(x3.shape))
        # x3 = self.decoder_low(x2)
        high1 = self.decoder_high(high)
        x5 = F.interpolate(x4, size=x.size()[-2:], mode='bilinear', align_corners=True)
        return self.encoder1(self.alise(self.alise2(torch.cat([x5, high1], dim=1)))) + x #残差链接
...
class HICENT(nn.Module):
    def __init__(self, upscale=4, conv=common.default_conv):
        ...
        #添加
        #self.attention = MultiHeadSelfAttention(n_feats=n_feats, num_heads=8)
    ...
    def forward(self, x1, x2=None, test=False):
        ...
        for i in range(self.n_blocks):
            x1 = self.body[i](x1)
            body_out.append(x1)
        res1 = torch.cat(body_out, 1)# [N, C_total, H', W']
