...
if __name__ == '__main__':
    ...
    print(f'Start a multiprocess pool with processes = {pool_num} for generating HiCENT data')
    results = []
    for n in chr_list:
    ...
    pool.close()
    pool.join()
    print(f'All HiCENT data generated. Running cost is {(time.time()-start)/60:.1f} min.')
    print(results)
    # return: n, div_dhic, div_hhic, div_inds, compact_idx, full_size
    ...
    filename = f'hicent_{high_res}{low_res}_c{chunk}_s{stride}_b{bound}_{pool_str}_{postfix}.npz'
    hicent_file = os.path.join(out_dir, filename)
    np.savez_compressed(hicent_file, data=data, target=target, inds=inds, compacts=compacts, sizes=sizes)
    print('Saving file:', hicent_file)
